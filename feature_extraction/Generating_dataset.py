...
if __name__ == '__main__':
    ...
    csv_output='./new_feature_csv/'
    pcapfiles = [f for f in os.listdir(input_directory) if f.endswith('.pcap')]
    subfiles_size = 20 # MB
    ...
    for i in range(len(pcapfiles)):
        ...
        pcap_file_directory=input_directory+pcap_file
        csv_output_directory=csv_output+pcap_file
        print(pcap_file)
        print(">>>> 1. splitting the .pcap file.")
        os.system('tcpdump -r '+ pcap_file_directory +' -w ' + split_directory + 'split_temp -C ' + str(subfiles_size))
        ...
        for f in tqdm(csv_subfiles):
            try:
                ...
                d.to_csv(csv_output_directory + '.csv', header=mode=='w', index=False, mode=mode)
                mode='a'
            except:
                pass
